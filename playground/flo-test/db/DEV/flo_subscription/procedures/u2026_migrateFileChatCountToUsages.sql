    ...
    WHILE pnCurrentMinId <= pnMaxUserId DO
        ...
        IF pnUserId IS NULL THEN
            ...
            LEFT JOIN (
                SELECT 
                    aat.user_id AS uid,
                    COALESCE(COUNT(DISTINCT sflc.id), 0) AS file_chat_count,
                    COALESCE(SUM(sf.size), 0) AS file_chat_bytes
                FROM flo_subscription.app_account_token aat
                ...
                LEFT JOIN preflow_41.storage_file sf ON sflc.file_uid = sf.uid
            ...
            INNER JOIN (
                ...
                FROM preflow_41.collection_notification_member cnm
                INNER JOIN preflow_41.storage_file_linked_collection sflc ON sflc.collection_id = cnm.channel_id
                INNER JOIN preflow_41.storage_file sf ON sflc.file_uid = sf.uid
