CREATE PROCEDURE `u2026_migrateCalendarBytesToUsages`(
    IN psUserEmail VARCHAR(255)
)
BEGIN
    ...
    WHILE pnCurrentMinId <= pnMaxUserId DO
        ...
        IF psUserEmail IS NULL THEN
            ...
            INNER JOIN (
                -- Calendar objects (events, journals, todos)
                SELECT 
                    usr.id AS user_id,
                    ...
                    COALESCE(SUM(CASE WHEN CONVERT(co.componenttype USING utf8mb4) = 'VEVENT' THEN 1 ELSE 0 END), 0) AS vevent_count,
                    COALESCE(SUM(CASE WHEN CONVERT(co.componenttype USING utf8mb4) = 'VJOURNAL' THEN 1 ELSE 0 END), 0) AS vjournal_count,
                    COALESCE(SUM(CASE WHEN CONVERT(co.componenttype USING utf8mb4) = 'VTODO' THEN 1 ELSE 0 END), 0) AS vtodo_count
                FROM preflow_41.user usr
                INNER JOIN preflow_41.calendarinstances ci 
                    ON ci.principaluri = CONCAT('principals/', usr.email)
                    AND ci.access = 1  -- owner only
            ...
            LEFT JOIN (
                -- Cards (separate subquery TO avoid row multiplication)
                SELECT 
                    usr.id AS user_id,
                    COUNT(DISTINCT cc.id) AS card_count
                FROM preflow_41.user usr
                LEFT JOIN preflow_41.addressbooks ab 
                    ON ab.principaluri = CONCAT('principals/', usr.email)
                LEFT JOIN preflow_41.card_contact cc 
                    ON cc.addressbookid = ab.id
                WHERE usr.id BETWEEN pnCurrentMinId AND pnCurrentMaxId
                GROUP BY usr.id
            ) card_data ON u.user_id = card_data.user_id
            ...
            SET 
                ...
                u.event_count = cal_data.vevent_count,
                u.note_count = cal_data.vjournal_count,
                u.todo_count = cal_data.vtodo_count,
                u.card_count = COALESCE(card_data.card_count, 0),
