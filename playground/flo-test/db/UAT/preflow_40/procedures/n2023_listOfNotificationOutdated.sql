...
LEFT JOIN user_notification un ON (cn.id = un.collection_notification_id AND un.user_id = co.user_id)
    ...
    WHERE cn.collection_id = IF(nCollectionID = 0, cn.collection_id, nCollectionID)
      AND co.id = IF(nCollectionID = 0, co.id, nCollectionID)
      -- 2. clean WHEN reach setting time LIMIT
      AND (nCollectionID = 0 AND cn.created_date < nNOW_AT_1AM - st.notification_clean_date)
   -- (co.id = nCollectionID
     --     OR cn.created_date < nNOW_AT_1AM - st.notification_clean_date)
      AND st.notification_clean_date > 0 -- <= 0: keep forever, < 0: save value WITH disable
...
LEFT JOIN user_notification un ON (cn.id = un.collection_notification_id AND un.user_id = csm.member_user_id)
     -- 1. immediately DELETE via worker collection
    WHERE cn.collection_id = IF(nCollectionID = 0, cn.collection_id, nCollectionID)
      AND csm.collection_id = IF(nCollectionID = 0, csm.collection_id, nCollectionID)
      -- 2. clean WHEN reach setting time LIMIT
      AND (nCollectionID = 0 AND cn.created_date < nNOW_AT_1AM - st.notification_clean_date)
   -- (csm.collection_id = nCollectionID
     --     OR cn.created_date < nNOW_AT_1AM - st.notification_clean_date)
