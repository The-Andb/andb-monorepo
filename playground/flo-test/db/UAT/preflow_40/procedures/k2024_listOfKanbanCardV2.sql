CREATE PROCEDURE `k2024_listOfKanbanCardV2`(pvIds         TEXT
...
BEGIN
  ...
  SELECT kc.id, kc.object_type, kc.item_card_order, kc.kanban_id, kc.account_id, kc.order_number
        ...
        ,kc.created_date, kc.updated_date
     ...
     ORDER BY 
         ...
         (CASE          
            WHEN NOT isnull(pnModifiedLT) AND isnull(pnModifiedGTE) THEN kc.updated_date
             --  ELSE kk.id
           END) DESC,
        (CASE WHEN NOT isnull(pnModifiedGTE) THEN kc.updated_date
              WHEN ifnull(pnMinId, 0) > 0 THEN kc.id
          END) ASC
         --
   LIMIT pnPageSize
  OFFSET nOFFSET;
