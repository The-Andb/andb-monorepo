...
BEGIN
  ...
  SELECT rcu.email -- , vGroupIds, bIsNoGroup
    ...
    --
      CASE -- string columns
    WHEN INSTR(vSort, '-') AND (vFieldSort='GROUPS') THEN gu.GROUPS
        WHEN INSTR(vSort, '-') AND (vFieldSort='email') THEN rcu.email
    END DESC,
      CASE -- NUMERIC columns
    WHEN INSTR(vSort, '-') AND (vFieldSort='id') THEN rcu.id
        WHEN INSTR(vSort, '-') AND (vFieldSort='account_3rd') THEN rcu.account_3rd
    WHEN INSTR(vSort, '-') AND (vFieldSort='storage') THEN rcu.storage_total
    WHEN INSTR(vSort, '-') AND (vFieldSort='subs_type') THEN rcu.subs_type
    WHEN INSTR(vSort, '-') AND (vFieldSort='last_used_date') THEN rcu.last_used_date
    WHEN INSTR(vSort, '-') AND (vFieldSort='join_date') THEN rcu.join_date
    WHEN INSTR(vSort, '-') AND (vFieldSort='next_renewal') THEN rcu.next_renewal
    END DESC,
    --
      CASE -- string columns
    WHEN INSTR(vSort, '+') AND (vFieldSort='GROUPS') THEN gu.GROUPS
