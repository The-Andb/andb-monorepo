CREATE FUNCTION `f2023_collectLinkedObjectSourceShared`(pvLinkType    VARCHAR(45)
...
BEGIN
  ...
  --
   ...
   SELECT lo.id, pvLinkType, lo.source_object_type, lo.source_object_uid, pnUserID, dNow, dNow
     FROM `user` u
     JOIN linked_object lo ON (u.id = lo.user_id 
                           AND lo.source_account_id = 0)
     JOIN linked_collection_object lco ON (lco.object_type = lo.source_object_type 
                                       AND lco.object_uid  = lo.source_object_uid)
