...
u2025_upsertUserUsageLog: BEGIN
  --
  ...
  --
  -- Skip IF user_id invalid
  IF IFNULL(pnUserId, 0) = 0 THEN
    LEAVE u2025_upsertUserUsageLog;
  END IF;
  --
  -- GET LIMIT BY plan
  ...
  SET nPercent = (nComValue / nLimit) * 100;
  --
  ...
  IF nID = 0 THEN
    ...
    IF nPercent >= nThreshold THEN
      INSERT INTO user_usage_warnings (user_id, component_id, percent, `value`, `LIMIT`)
      VALUES (pnUserId, pnComponentId, nPercent, nComValue, nLimit);
    END IF;
    --
  ELSE
