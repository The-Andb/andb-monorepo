CREATE PROCEDURE `testu2025_subscriptionUsageInUse`(
    IN pnUserID BIGINT
)
BEGIN
  --
  EXPLAIN ANALYZE WITH target_orders AS (
  SELECT o.id
      ,o.plan_id
      ,o.user_id
      ,o.name
  ...
  SELECT o.id order_id, o.plan_id, o.user_id
    ...
    JOIN usages u ON (u.user_id = o.user_id AND u.is_active = 1 AND u.component_id = c.id)
    LEFT JOIN purchases p ON (p.order_id = o.id
                  AND p.user_id = o.user_id
                  AND p.is_current = 1
                 )
    LEFT JOIN transactions t ON (t.id = p.transaction_id)
    LEFT JOIN alert a_reach ON (a_reach.user_id = o.user_id
                  AND a_reach.component_id = c.id
                  AND a_reach.limit_type = 'REACH'
                  AND a_reach.deleted_at IS NULL
                   )
    LEFT JOIN alert a_exceed ON (a_exceed.user_id = o.user_id
