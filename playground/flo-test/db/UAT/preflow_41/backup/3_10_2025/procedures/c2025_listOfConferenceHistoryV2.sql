CREATE PROCEDURE `c2025_listOfConferenceHistoryV2`(pvKeyword         TEXT
                                                                   ,pvChannelIDs      TEXT
                                                                   ,pvIDs             TEXT
                                                                   ,pnMinId           BIGINT(20)
...
BEGIN
  ...
  SELECT ch.id, ch.invitee, ch.phone_number, ifnull(ch.attendees, '[]') lsAttendees
        ...
        ,ch1.created_date, ch1.updated_date
    ...
    JOIN (
      SELECT max(ch.id) id
         ...
         ,max(ch.created_date) created_date, max(ch.updated_date) updated_date
        FROM conference_history ch
    ...
    JOIN conference_channel cc ON (cm.channel_id = cc.id)
   WHERE ch.user_id = pnUserId
     AND ch1.updated_date  <  IF(ifnull(pnModifiedLT,    0) > 0, pnModifiedLT, unix_timestamp() + 1)
     AND ch1.updated_date  >= IF(ifnull(pnModifiedGTE,   0) > 0, pnModifiedGTE, 0)
     AND ch1.created_date  <  IF(ifnull(pnCreatedBefore, 0) > 0, pnCreatedBefore, unix_timestamp() + 1)
     AND ch1.created_date  > IF(ifnull(pnCreatedAfter,  0) > 0, pnCreatedAfter, 0)
     AND (pnMinId IS NULL OR ch.id > pnMinId)
     AND (pvIDs IS NULL OR FIND_IN_SET(ch.id, pvIDs))
     AND (pvChannelIDs IS NULL OR FIND_IN_SET(cc.id, pvChannelIDs))
   ORDER BY 
