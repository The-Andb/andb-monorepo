CREATE PROCEDURE `C2025_listOfCollectionTree`(
  ...
  pnFilterCreatedDateLT DOUBLE (14, 4),
...
BEGIN
  ...
  WITH collection_empty_finder AS (
    ...
    WHERE
      ...
      AND `c`.`created_date` < pnFilterCreatedDateLT
      ...
      AND EXISTS (
        ...
        WHERE
          ...
          AND (ISNULL(pvObjectType) OR pvObjectType = '' OR FIND_IN_SET(lco.object_type, pvObjectType) = 0)
      )
  ),
  ...
  SELECT
    ...
    view_mode
  FROM
    hierarchy_collections hc
  WHERE hc.hierarchy_level = (
    SELECT MIN(hc2.hierarchy_level)
    FROM hierarchy_collections hc2
    WHERE hc2.co_id = hc.co_id
  )
