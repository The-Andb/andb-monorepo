...
BEGIN
  ...
  IF pvObjectType = 'VTODO' THEN
    ...
    IF nReturn > 0 THEN
      --
      SET bUnassigned = bUnassigned OR vAssigned = 0;
      --
      SET bAssignedByMe = LOCATE(pvUsername, vOrganizer) > 0;
      --
      SET bAssignedToMe = LOCATE(pvUsername, vRealAssignees) > 0;
      --
      SET bAssignedByOther = LOCATE(pvUsername, vRealAssignees) = 0 AND LOCATE(pvUsername, vOrganizer) = 0;
      --
    END IF;
    -- 
  END IF;
