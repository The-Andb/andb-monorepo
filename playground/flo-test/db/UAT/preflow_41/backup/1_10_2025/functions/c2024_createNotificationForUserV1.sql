CREATE FUNCTION `c2024_createNotificationForUserV1`(pnUserId             BIGINT(20)
...
BEGIN
 ...
 -- Only CREATE for owner comment
  IF ifnull(pvObjectType, '') = 'COMMENT' AND ifnull(pnAction, 0) = 24 THEN
    SELECT cc.user_id, cc.email, ca.object_type
      INTO nOwnerUserId, vOwnerEmail, vObjectType
  ...
  INSERT INTO collection_notification
    ...
    `action`, action_time, assignees, content, kanban_id, created_date, updated_date, channel_id, emoji_unicode, account_id)
  VALUES
    (nOwnerUserId, vOwnerEmail, pvUsername, nCollectionId, pnCommentId, pvObjectUid, ifnull(vObjectType, pvObjectType)
    ,pnAction, pnActionTime, '', ifnull(pvContent, ''), 0, pnUpdatedDate, pnUpdatedDate, pnChannelId, pvEmojiUnicode, pnAccountId);
  --
