...
BEGIN
  ...
  INSERT INTO collection_notification
    ...
    `action`, action_time, content, created_date, updated_date, channel_id)
  VALUES
  ...
  IF pvObjectType IN ('CHAT', 'COMMENT') THEN
    SET nReturn = n2023_createUserNotification(nNotiID, 0, 0, pnActionTime, pnUpdatedDate, pnUpdatedDate, NULL, pnUserId, pvUsername);
  ...
  IF pnAction = 30 AND ifnull(pvMentionList, '') <> ''THEN
    --
    SET nReturn = n2024_considerMentionInNotificationForChatV3(NULL, nNotiID, pvObjectUid, pvObjectType, pnUserId, pvMentionList, pnActionTime, pnUpdatedDate);
    -- 
  END IF;
  --
  RETURN nNotiID;
