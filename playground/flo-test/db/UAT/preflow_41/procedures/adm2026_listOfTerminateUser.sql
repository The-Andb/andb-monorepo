...
BEGIN
  ...
  SELECT COUNT(DISTINCT uu.id) INTO nRows
   ...
   WHERE (rcu.email LIKE vKeyword OR rcu.account_3rd_emails LIKE vKeyword)
     ...
     AND IF(ifnull(pvStatuses, '') = '', 1, 
         FIND_IN_SET(
             CASE 
                 WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) = -1 THEN 1
                 WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) > 0 THEN 2
                 WHEN ifnull(ud.progress, 0) = 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 3
                 WHEN ifnull(ud.progress, 0) > 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 4
                 ELSE 0
             END, 
             pvStatuses
         )
  ...
  SELECT
        ...
        ,CASE 
            WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) = 0 THEN 1  -- Waiting for approval
            WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) > 0 THEN 2  -- Pending Deletion
            WHEN ifnull(ud.progress, 0) = 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 3  -- Cleaning STARTING
            WHEN ifnull(ud.progress, 0) > 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 4  -- Cleaning IN progress
            ELSE 0
    ...
    LEFT JOIN (
     ...
     WHERE (rcu.email LIKE vKeyword OR rcu.account_3rd_emails LIKE vKeyword)
       ...
       AND IF(ifnull(pvStatuses, '') = '', 1, 
           FIND_IN_SET(
               CASE 
                   WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) = 0 THEN 1
                   WHEN ifnull(ud.progress, 0) = 0 AND ifnull(ud.cleaning_date, -1) > 0 THEN 2
                   WHEN ifnull(ud.progress, 0) = 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 3
                   WHEN ifnull(ud.progress, 0) > 1 AND ifnull(ud.cleaning_date, -1) > 0 THEN 4
                   ELSE 0
          ...
          CASE 
            WHEN INSTR(vSort, '-') AND vFieldSort IN ('id', 'progress', 'cleaning_date', 'created_date') THEN 
              CASE vFieldSort 
                ...
                WHEN 'cleaning_date' THEN ifnull(ud.cleaning_date, -1)
