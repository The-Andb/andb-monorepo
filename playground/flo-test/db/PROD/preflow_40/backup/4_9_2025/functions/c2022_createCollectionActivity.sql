CREATE FUNCTION `c2022_createCollectionActivity`(nCollectionId         BIGINT(20)
                                                               ,objectUid            VARBINARY(1000)
                                                               ,objectType           VARBINARY(50)
                                                               ,createdDate          DOUBLE(13,3)
                                                               ,updatedDate          DOUBLE(13,3)) RETURNS BIGINT
BEGIN
  --
  ...
  ELSE
   ...
   WHERE co.id = nCollectionId;
    --
    IF ifnull(objectType, '') NOT IN('KANBAN' ,'URL') THEN
      --
    ...
    (`collection_id`, old_collection_id, user_id, `object_uid`, `object_type`, object_href,  `created_date`, `updated_date`)
    VALUES
    (nCollectionId, nCollectionId, nUserId, objectUid, objectType, vObjectHref, createdDate, updatedDate);
