CREATE PROCEDURE `c2025_listOfCollectionHistory`(IN pnCollectionId BIGINT,
                                                                 IN pvObjectUid VARBINARY(1000), IN pnUserId BIGINT,
                                                                 IN pvIDs TEXT, IN pnModifiedGTE DOUBLE(14, 4),
                                                                 IN pnModifiedLT DOUBLE(14, 4), IN pnMinId BIGINT,
                                                                 IN pnPageSize INT, IN pnPageNo INT,
                                                                 IN pvSort VARCHAR(128), IN pnIncludeShared TINYINT(1))
BEGIN
  --
  ...
  SELECT his.id, his.collection_activity_id, his.email, his.action, his.action_time, his.content
    ...
    FROM (
          ...
          SELECT ch.id, ch.collection_activity_id, ch.email, ch.action, ch.action_time, ch.content
            ...
            JOIN user usr ON (co.user_id = usr.id)
           WHERE (nCollectionId = 0 OR co.id = nCollectionId)
             AND (csm.member_user_id = pnUserId OR csm.user_id = pnUserId)
             AND ca.object_type = 'KANBAN'
             AND co.is_trashed = 0
             AND co.type = 3 -- share only
