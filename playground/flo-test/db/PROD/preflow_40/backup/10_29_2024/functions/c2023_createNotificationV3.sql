CREATE FUNCTION `c2023_createNotificationV3`(pnUserId             BIGINT(20)
...
BEGIN
  ...
  INSERT INTO collection_notification
    ...
    `action`, action_time, assignees, content, kanban_id, kanban_card_id, created_date, updated_date)
  VALUES
    (pnUserId, pvUsername, pnCollectionId, CASE WHEN pnAction IN (6, 61, 62) THEN pnCommentId ELSE 0 END, pvObjectUid, pvObjectType
    ,pnAction, pnActionTime, CASE WHEN pnAction IN (17, 18) THEN ifnull(pvAssignee, '') ELSE '' END, ifnull(vContent, ''), pnKanbanID, pnKanbanCardID, pnUpdatedDate, pnUpdatedDate);
  --
