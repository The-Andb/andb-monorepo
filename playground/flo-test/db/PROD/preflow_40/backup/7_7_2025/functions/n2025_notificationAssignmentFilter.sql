...
BEGIN
  ...
  -- 17 >> Assigned
  DECLARE bAsignedToMe  TINYINT(1) DEFAULT (pnAction = 17
  ...
  RETURN find_in_set(0, pvAssignmentFilter)
             ...
             pvObjectType = 'VTODO' AND (
               ...
               -- 1: NOT Assigned
               OR (find_in_set(1, pvAssignmentFilter) 
                   AND bUnassigned)
               -- 2: Assigned TO Me 
               OR (find_in_set(2, pvAssignmentFilter)
                   AND bAsignedToMe)
               -- 3: Assigned BY Me
               OR (find_in_set(3, pvAssignmentFilter) 
                   AND bAssignedByMe)
               -- -1: Other
               OR (FIND_IN_SET(-1, pvAssignmentFilter)
                  AND bAssignedOther)
