...
BEGIN
  ...
  SELECT cm.id, ifnull(cc.collection_id, 0) collection_id, cm.channel_id, cc.uid, cm.email, cm.is_creator, cm.description
        ...
        /*,IF(ifnull(cc.collection_id,0) > 0
            ,cc.title -- always USE share title for shared channel
            ,IFNULL(cm.title, '')) title*/
        ,cc.title share_title
        ,ch.start_time, ch.end_time, ch.action_time, ch.status, ch.type
        ,cc.last_used
        ,cc.realtime_channel
        -- ,GREATEST(ifnull(ch.updated_date, 0), cc.created_date, ifnull(cc.last_used, 0)) vS
...
-- LEFT JOIN collection co ON (co.id = cc.collection_id)
    WHERE cm.user_id = pnUserId
      ...
      -- value = 3,4,5,6 >> search BY channel name || participant 
      AND (IF(nFilterType IN (3,4,5,6,7)
           -- search BY title
        ...
        -- value = 4 >> 3 + missed CALL included (declined)
        -- AND IF(nFilterType = 4, ch.status IN (23, 24, 25) AND ch.end_time > 0, 1)
        AND IF(nFilterType = 4
            ,(cm.missed_calls > 0 AND cm.last_missed_call >= UNIX_TIMESTAMP(NOW(3) - INTERVAL 1 DAY))
          OR (ch.status IN (23, 24, 25) AND ch.end_time >= UNIX_TIMESTAMP(NOW(3) - INTERVAL 1 DAY))
            ,1)
        -- 
      ...
      ORDER BY
        (CASE
           ...
           WHEN INSTR(vSort, "-") AND ifnull(pnPageNo, 0) > 0
             THEN -- DEFAULT WHEN no ASC active
             --
             CASE vFieldSort
               --
               WHEN 'start_time'   THEN GREATEST(ifnull(ch.updated_date, 0), cm.created_date, ifnull(cc.last_used, 0))
