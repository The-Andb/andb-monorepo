...
BEGIN
  ...
  IF (nCollectionId >= 0 AND nCollectionId <> pnCollectionId) THEN
     ...
     SET nUpdatedDate = pdUpdatedDate + (countMax + 1 ) * 0.001;
     --
     UPDATE collection_activity ca 
      ...
      WHERE ca.id      = pnCollectionActivityId 
        AND ca.user_id = pnUserId;
     -- CREATE notification 
     SET nReturn = c2024_createNotiWhenMove(pnUserId, pvUserEmail, nCollectionId, pnCollectionId, oldObjectUid, pvObjectUid, vObjectType, pvContent, pnActionTime, nUpdatedDate);
     --
     SELECT cn.content
       INTO vObjectTitle
