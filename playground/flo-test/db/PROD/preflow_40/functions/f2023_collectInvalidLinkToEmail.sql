CREATE FUNCTION `f2023_collectInvalidLinkToEmail`(pvObjectUid   VARBINARY(1000)
                                                                    ,pnUserID      BIGINT(20)
                                                                    ,pvEmail       VARCHAR(255)
                                                                    ) RETURNS INT
BEGIN
 ...
 LEFT JOIN user_process_invalid_link upil ON (u.id = upil.user_id)
  ...
  IF nID = 0 THEN 
    ...
    SET nID = f2023_updateUserProcessInvalidData(NULL, nUserID, pvEmail, 0, 0, NULL, NULL);
    --
  END IF;
  --
   INSERT INTO flo_invalid_link
         (link_id, link_type, object_type, object_uid, user_id, created_date, updated_date)
   SELECT ch.id, 'CH', ch.destination_object_type, ch.destination_object_uid, ch.user_id, dNow, dNow
     FROM user u
    ...
    WHERE ch.user_id                 = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = ch.user_id 
                AND fli.object_uid = ch.destination_object_uid 
                AND fli.object_type = ch.destination_object_type
                AND fli.link_id = ch.id
            )
    UNION
   SELECT ch.id, 'CH', ch.source_object_type, ch.source_object_uid, ch.user_id, dNow, dNow
     FROM user u
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = ch.user_id 
                AND fli.object_uid = ch.source_object_uid 
                AND fli.object_type = ch.source_object_type
                AND fli.link_id = ch.id
            )
    UNION
   SELECT kc.id, 'KC', kc.object_type, kc.object_uid, kc.user_id, dNow, dNow
     FROM user u
    ...
    WHERE kc.user_id            = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = kc.user_id 
                AND fli.object_uid = kc.object_uid 
                AND fli.object_type = kc.object_type
                AND fli.link_id = kc.id
            )
    UNION
   SELECT lco.id, 'LCO', lco.object_type, lco.object_uid, lco.user_id, dNow, dNow
     FROM user u
    ...
    WHERE lco.user_id             = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = lco.user_id 
                AND fli.object_uid = lco.object_uid 
                AND fli.object_type = lco.object_type
                AND fli.link_id = lco.id
            )
    UNION         
   SELECT lo.id, 'LO', lo.destination_object_type, lo.destination_object_uid, lo.user_id, dNow, dNow
     FROM user u
    ...
    WHERE lo.user_id            = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = lo.user_id 
                AND fli.object_uid = lo.destination_object_uid 
                AND fli.object_type = lo.destination_object_type
                AND fli.link_id = lo.id
            )
    UNION     
   SELECT lo.id, 'LO', lo.source_object_type, lo.source_object_uid, lo.user_id, dNow, dNow
     FROM user u
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = lo.user_id 
                AND fli.object_uid = lo.source_object_uid 
                AND fli.object_type = lo.source_object_type
                AND fli.link_id = lo.id
            )
    UNION       
   SELECT tc.id, 'TC', tc.object_type, tc.object_uid, tc.user_id, dNow, dNow
     FROM user u
    ...
    WHERE tc.user_id            = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = tc.user_id 
                AND fli.object_uid = tc.object_uid 
                AND fli.object_type = tc.object_type
                AND fli.link_id = tc.id
            )
    UNION       
   SELECT et.id, 'ET', et.object_type, et.object_uid, et.user_id, dNow, dNow
     FROM user u
    ...
    WHERE et.user_id            = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = et.user_id 
                AND fli.object_uid = et.object_uid 
                AND fli.object_type = et.object_type
                AND fli.link_id = et.id
            )
    UNION       
   SELECT me.id, 'ME', me.object_type, me.object_uid, me.user_id, dNow, dNow
     FROM user u
    ...
    WHERE me.user_id            = nUserID
     ...
     -- NOT EXISTS FIL aka inserted BEFORE
      AND NOT EXISTS 
            (SELECT 1
               FROM flo_invalid_link fli
              WHERE fli.user_id = me.user_id 
                AND fli.object_uid = me.object_uid 
                AND fli.object_type = me.object_type
                AND fli.link_id = me.id
            )
     ON DUPLICATE KEY UPDATE updated_date=VALUES(updated_date);
            
  -- immediate DELETE after received sinal FROM IMAP server that email has been deleted
  -- SET nReturn = f2023_cleanInvalidLinks4User(nUserID);
  -- public via queue
