...
BEGIN
  ...
  FETCH activity_cursor
    ...
    -- UPDATE new collection for history AND comment
  ...
  --   
    ...
    IF vTable = 'history' THEN
      ...
      UPDATE collection_history ch
         SET ch.created_date = CASE 
                                    WHEN ch.created_date = ch.updated_date THEN nDateItemHistory 
                                    ELSE ch.created_date 
                                END
             ,ch.updated_date = nDateItemHistory
             ,ch.collection_id = nCollectionId
        WHERE ch.id = nID;
    ...
    ELSEIF vTable = 'comment' THEN
      ...
      UPDATE collection_comment cc 
         SET cc.created_date = CASE 
                                    WHEN cc.created_date = cc.updated_date THEN nDateItemComment 
                                    ELSE cc.created_date 
                                END
            ,cc.updated_date = nDateItemComment
            ,cc.collection_id = nCollectionId
       WHERE cc.id = nID;
      --
