CREATE FUNCTION `c2023_createConferenceHistorySendInvite`(pnChannelId  BIGINT(20)
                                                                            ,pnUserId     BIGINT(20)
                                                                            ,pnType       TINYINT(1)
                                                                            ,pvOrganizer  VARCHAR(100)
                                                                            ,pvInvitees   TEXT
                                                                            ,pvMeetingId  VARCHAR(1000)) RETURNS INT
BEGIN
  -- DEPRECATED
  ...
  INSERT INTO conference_history
        (user_id, member_id, invitee, start_time, type, status, organizer, 
         action_time, meeting_id, created_date, updated_date) 
  SELECT u.id, cm.id, u.username, unix_timestamp(now(3)), pnType, 24, pvOrganizer
        ,unix_timestamp(now(3)), pvMeetingId, unix_timestamp(now(3)), unix_timestamp(now(3))
