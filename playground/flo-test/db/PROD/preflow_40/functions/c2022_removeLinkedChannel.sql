CREATE FUNCTION `c2022_removeLinkedChannel`(uid         VARBINARY(1000)
                                                                   ,nUserId      BIGINT(20)
                                                                   ,dDeleteTime  DOUBLE(13,3)) RETURNS INT
BEGIN
  --
 ...
 SELECT DISTINCT(lo.id), 'OBJ' ltype
  ...
  WHERE lo.user_id = nUserId
    AND ((lo.source_object_uid = uid AND lo.source_object_type = 'CONFERENCING')
        OR (lo.destination_object_uid = uid AND lo.destination_object_type = 'CONFERENCING'))
  UNION
  -- 3. `kanban_card`
 ...
 SELECT DISTINCT(ch.id), 'HIS' ltype
  ...
  link_loop: LOOP
    ...
    IF vType = 'COL' THEN 
      ...
      SET nReturn = m2023_insertAPILastModify('linked_collection_object', nUserId, dDeleteTime);
    ...
    ELSEIF vType = 'OBJ' THEN
      ...
      SET nReturn = m2023_insertAPILastModify('linked_object', nUserId, dDeleteTime);
    ...
    ELSEIF vType = 'KAN' THEN
      ...
      AND id  = nID;
      --
      SET nReturn = m2023_insertAPILastModify('kanban', nUserId, dDeleteTime);
      --
    ELSEIF vType = 'HIS' THEN
      ...
      AND id  = nID;
      --
      SET nReturn = m2023_insertAPILastModify('conference_history', nUserId, dDeleteTime);
      --
    END IF;
    --
    SET nCount = nCount + 1;
