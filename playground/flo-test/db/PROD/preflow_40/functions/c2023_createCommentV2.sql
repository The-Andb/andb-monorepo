CREATE FUNCTION `c2023_createCommentV2`(pnCollectionId         BIGINT(20)
...
BEGIN
  ...
  SELECT ifnull(u.email, '')
    ...
    INSERT INTO  collection_comment
      (collection_activity_id, email, `action`, action_time, `comment`, is_draft, user_id, parent_id, created_date, updated_date)
    VALUES
      (nCAID, vEmail, 0, pnActionTime, pvComment, ifnull(pnDraft, 0), pnUserId, pnParentId, pnCreatedDate, pnUpdatedDate)
