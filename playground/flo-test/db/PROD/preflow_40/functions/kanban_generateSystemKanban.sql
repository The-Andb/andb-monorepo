CREATE FUNCTION `kanban_generateSystemKanban`(pnUserId           BIGINT(20)
...
BEGIN
  ...
  SELECT greatest(unix_timestamp(now(3)), ifnull(max(k.updated_date), 0))
    INTO nUpdatedDate
    FROM kanban k
    ...
    IF nIsOwner = 1 THEN
     ...
     ELSE
       ...
       VALUES
         ...
         -- Calls
         (pnUserId, pnCollectionId,        'ToDo\'s', '#7CCD2D',    2, 0, NULL,  nUpdatedDate + 0.003, 0, 0, 0, 0.000, 1, 0,  nUpdatedDate + 0.004,  nUpdatedDate + 0.004),
         -- ToDo
         (pnUserId, pnCollectionId,          'Calls', '#49BB89',    3, 0, NULL,  nUpdatedDate + 0.004, 0, 0, 3, 0.000, 1, 0,  nUpdatedDate + 0.005,  nUpdatedDate + 0.005),
         -- Notes
  ...
  END IF;
     ...
     -- Calls
         (pnUserId, pnCollectionId,          'Calls', '#49BB89', 6, 0, NULL,  nUpdatedDate + 0.014, 0, 0, 3, 0.000, 1, 0,  nUpdatedDate + 0.014,  nUpdatedDate + 0.014),
         -- Notes
