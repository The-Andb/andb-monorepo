CREATE PROCEDURE `n2024_listOfNotificationV3`(
   ...
   SELECT cn.id, cn.email, cn.collection_id, cn.object_uid, cn.object_type
        -- VIRTUAL action: 63 - MENTIONED
         ,CASE
            ...
            THEN 63
            WHEN (cn.action = 23) AND un.has_mention = 1
            THEN 25
            ELSE cn.action
         END `action`
         ...
         ,IF(cn.object_type IN ('URL', 'CHAT', 'COMMENT') ,'', concat("/calendarserver.php/calendars/"
...
--       AND IF(ifnull(pnIncludeShared, 0) = 1, 1, cn.`action` NOT IN (20,21,22))
      ...
      AND (find_in_set(0, vAssignment)
       ...
       AND IF(ifnull(pnIncludeReaction, 0) IN (1,3) 
          ...
          OR ifnull(pvAction, 0) = 23, 1, cn.`action` NOT IN (23)
      )
       AND IF(ifnull(pnIncludeReaction, 0) IN (2,3) 
          OR ifnull(pvObjectType, '') = 'COMMENT' 
          OR ifnull(pvAction, 0) = 24, 1, cn.`action` NOT IN (24)
      )
   GROUP BY cn.id
   ORDER BY
        (CASE
