CREATE PROCEDURE `c2024_listOfConferenceV2`(pvKeyword       TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci
...
BEGIN
  ...
  SELECT cm.id, cm.channel_id, cc.uid, cm.email, cm.is_creator, cm.description
        ...
        ,cc.last_used
        ,cc.realtime_channel
        -- ,GREATEST(ifnull(ch.updated_date, 0), cc.created_date, ifnull(cc.last_used, 0)) vSort
...
LEFT JOIN linked_collection_object lco ON (cc.uid = lco.object_uid 
    ...
    WHERE cm.user_id = pnUserId
      ...
      AND IF(pnMinId IS NULL ,1 , cm.id > pnMinId)
      ...
      AND (IF(nFilterType IN (3,4,5,6)
        ...
        -- AND IF(nFilterType = 4, ch.status IN (23, 24) AND ch.end_time > 0, 1)
        AND IF(nFilterType = 4
            ,cm.missed_calls > 0
             OR (ch.status IN (23, 24) AND ch.end_time > 0)
             ,1)
        -- 
