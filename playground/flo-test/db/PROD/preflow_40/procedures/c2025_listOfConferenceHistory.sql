CREATE PROCEDURE `c2025_listOfConferenceHistory`(pvKeyword           TEXT
                                                                   ,pvChannelIDs        TEXT
                                                                   ,pnUserId            BIGINT(20)
                                                                   ,pvIDs               TEXT
...
BEGIN
  ...
  SELECT ch.id, ch.invitee, ch.phone_number, ifnull(ch.attendees, '[]') lsAttendees
   ...
   WHERE ch.user_id = pnUserId
     ...
     AND IF(pvIDs IS NOT NULL, FIND_IN_SET(ch.id, pvIDs), 1)
     AND IF(pvChannelIDs IS NOT NULL, FIND_IN_SET(cc.id, pvChannelIDs), 1)
     AND (pvMeetingID IS NULL OR ch.meeting_id = pvMeetingID)
     AND (pvExternalMeetingID IS NULL OR ch.external_meeting_id = pvExternalMeetingID)
   ORDER BY 
    --
