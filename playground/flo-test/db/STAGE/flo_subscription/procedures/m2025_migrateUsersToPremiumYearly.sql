...
main_block: BEGIN
    ...
    DECLARE pnMigrationStart INT DEFAULT UNIX_TIMESTAMP();
    ...
    -- IF psBasePlan IS specified, only migrate users currently ON that plan
    DECLARE user_cursor CURSOR FOR
        ...
        LEFT JOIN flo_subscription.orders o ON o.id = p.order_id
        ...
        WHERE ud.id IS NULL
          ...
          AND (psBasePlan IS NULL OR cur_pl.name = psBasePlan OR (psBasePlan IS NOT NULL AND cur_pl.name IS NULL));
    ...
    -- IF psBasePlan IS specified, only count users currently ON that plan
    -- ========================================================================
    ...
    SELECT 
        ...
        IFNULL(psBasePlan, 'ALL PLANS') AS base_plan_filter,
    ...
    user_loop: LOOP
        ...
        -- Step 1: UPDATE report_cached_user (WITH order_number)
        ...
        WHERE LOWER(TRIM(rcu.email)) = LOWER(TRIM(psCurrentEmail));
        ...
        -- Step 4a: Remove alerts for this user
        SET psCurrentStep = 'Step4a_RemoveAlerts';
        DELETE FROM flo_subscription.alert
        WHERE user_id = pnCurrentUserId;
        
        -- Step 4b: Remove user_usage_warnings for this user
        SET psCurrentStep = 'Step4b_RemoveUserUsageWarnings';
        DELETE FROM flo_subscription.user_usage_warnings
        WHERE user_id = pnCurrentUserId;
        
        -- Step 5: CREATE new ORDER
        SET psCurrentStep = 'Step5_CreateNewOrder';
        ...
        WHERE pl.product_id = psSubId AND pl.circle_life = 'yearly' AND pl.plan_group = 3
        LIMIT 1;
        
        -- Step 6: CREATE order_details
        SET psCurrentStep = 'Step6_CreateOrderDetails';
        INSERT INTO flo_subscription.order_details (order_id, component_id, component_value, description, created_date, updated_date)
        SELECT o.id, pd.component_id, pd.component_value, pd.description, NOW(), NOW()
        FROM flo_subscription.orders o
        INNER JOIN flo_subscription.plans pl ON o.plan_id = pl.id
        INNER JOIN flo_subscription.plan_details pd ON pd.plan_id = pl.id
        WHERE o.user_id = pnCurrentUserId
          AND o.is_active = 1 
          AND o.created_date >= FROM_UNIXTIME(pnMigrationStart);
        
        -- Step 7: CREATE new usages (WITH transferred VALUES FROM old usages)
        -- Transfer existing used_value AND used_data FROM old usages IF they exist
        SET psCurrentStep = 'Step7_CreateNewUsages';
        ...
        SELECT 
            pnCurrentUserId, 
            pd.component_id,
            -- Transfer used_value: USE old value IF EXISTS, otherwise USE plan DEFAULT
            COALESCE(old_us.used_value, pd.component_value) AS used_value,
            -- Transfer used_data: USE old value IF EXISTS
            old_us.used_data AS used_data,
            pd.description, 
            1, 
            NOW(), 
            NOW()
        FROM flo_subscription.plan_details pd
        INNER JOIN flo_subscription.plans pl ON pd.plan_id = pl.id
        LEFT JOIN flo_subscription.usages old_us ON old_us.user_id = pnCurrentUserId 
            AND old_us.component_id = pd.component_id 
            AND old_us.is_active = 0
            AND old_us.updated_date >= FROM_UNIXTIME(pnMigrationStart)
        WHERE pl.product_id = psSubId AND pl.circle_life = 'yearly' AND pl.plan_group = 3;
        
        -- Step 8: CREATE new purchase
        SET psCurrentStep = 'Step8_CreateNewPurchase';
        ...
        SELECT pnCurrentUserId, o.id, 0, pl.description, psPurchaseDate, 1, 4, psSubId, 1, psExpiresAt, psExpiresAt, psPurchaseDate, psExpiresAt, '', NOW(), NOW()
        FROM flo_subscription.orders o
