...
BEGIN
    ...
    INNER JOIN preflow_40.user u_old ON u.user_id = u_old.id
    INNER JOIN preflow_40.quota q ON u_old.username = q.username
    SET 
    ...
    INNER JOIN (
        ...
        FROM preflow_40.user usr
        INNER JOIN preflow_40.calendarinstances ci 
            ON ci.principaluri = CONCAT('principals/', usr.email)
            AND ci.access = 1
        INNER JOIN preflow_40.calendarobjects co 
