...
BEGIN
  ...
  INSERT INTO collection_notification_member_temp 
...
LEFT JOIN collection_notification_member_temp cnm ON (cnm.collection_id = co.id AND cnm.channel_id = co.channel_id AND cnm.member_user_id = co.user_id)
...
--     JOIN calendarinstances ci ON (co.calendar_uri = ci.uri)
    JOIN user usr ON co.user_id = usr.id
    LEFT JOIN collection_notification_member_temp cnm ON (cnm.collection_id = co.id AND cnm.channel_id = co.channel_id AND cnm.member_user_id = csm.member_user_id)
  ...
  SELECT ifnull(cc.collection_id, 0) collection_id, 0 calendarid
    ...
    JOIN `user` u ON (u.id = cm.user_id)
     LEFT JOIN collection_notification_member_temp cnm ON (cnm.collection_id = cc.collection_id AND cnm.channel_id = cc.id AND cnm.member_user_id = cm.user_id)
  ...
  SELECT 0 AS collection_id, 0 calendarid
        ...
        ,unix_timestamp() AS updated_date
    FROM user uu
    LEFT JOIN collection_notification_member_temp cnm ON (cnm.collection_id = 0 AND cnm.channel_id = 0 AND cnm.member_user_id = uu.id)
