CREATE FUNCTION `OTE_migrateRealTimeCollectionMember`() RETURNS INT(11)
BEGIN
 ...
 LEFT JOIN (
           ...
           ) rcm ON (u.email = rcm.email AND co.id = rcm.internal_channel_id)
 ...
 LEFT JOIN (
           SELECT rcm.id, rcm.email, rc.internal_channel_id
             ...
             JOIN realtime_channel rc ON (rcm.channel_id = rc.id)
            WHERE rc.type = 'COLLECTION'
           ) rcm ON (csm.shared_email = rcm.email AND csm.collection_id = rcm.internal_channel_id)
      WHERE rcm.id IS NULL
        AND rc.type = 'COLLECTION'
      ;
       -- LIMIT 10000
