CREATE PROCEDURE `c2023_createConferenceHistorySendInviteV2`(pnChannelId  BIGINT(20)
                                                                            ,pnUserId       BIGINT(20)
                                                                            ,pnType         TINYINT(1)
  ...
  his_loop: LOOP
    ...
    SELECT ifnull(max(cm.id), 0)
     ...
     WHERE cm.email = vEmail
       AND cm.channel_id = pnChannelId
       AND find_in_set(cm.email, pvInvitees)
       ; 
    --
    IF nID = 0 OR nForceCall THEN
      ...
      --
      SET vReturn = concat(vReturn, vEmail, ',');
      --
    END IF;
    --
    SET vMembers = concat(vMembers, vEmail, ',');
    --
    # main INSERT
  END LOOP his_loop;
  --
  SELECT vReturn invitees, vMembers members;
  --
END