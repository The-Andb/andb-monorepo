CREATE FUNCTION `f2023_collectRecentObjectCalDAV`(pvLinkType    VARCHAR(45)
...
BEGIN
  ...
  --
   ...
   SELECT ro.id, pvLinkType, ro.object_type, ro.object_uid, ro.user_id, dNow, dNow
     FROM recent_object ro
     -- OUTER calendar object aka Invalid link
LEFT JOIN calendarobjects co ON (ro.object_uid = co.uid)
    WHERE ro.user_id     = ifnull(pnUserID, ro.user_id)
      ...
      AND ro.object_type IN ('VTODO', 'VEVENT', 'VJOURNAL')
      AND co.id IS NULL
