CREATE FUNCTION `f2023_collectContactHistorySourceCalDAV`(pvLinkType    VARCHAR(45)
...
LEFT JOIN calendarobjects co ON (ch.source_object_uid = co.uid)  
    WHERE ch.user_id     = ifnull(pnUserID, ch.user_id)
      ...
      AND ch.source_object_type  = 'VEVENT' -- IN ('VTODO', 'VEVENT', 'VJOURNAL')
      AND co.id IS NULL
