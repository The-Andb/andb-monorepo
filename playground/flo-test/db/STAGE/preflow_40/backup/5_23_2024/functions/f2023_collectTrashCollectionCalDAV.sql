CREATE FUNCTION `f2023_collectTrashCollectionCalDAV`(pvLinkType    VARCHAR(45)
...
LEFT JOIN calendarobjects co ON (tc.object_uid = co.uid)
    WHERE tc.user_id     = ifnull(pnUserID, tc.user_id)
      AND tc.object_uid  = ifnull(pvObjectUid, tc.object_uid)
      AND tc.object_type IN ('VTODO', 'VEVENT', 'VJOURNAL')
      AND co.id IS NULL
