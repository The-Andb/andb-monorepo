CREATE FUNCTION `c2024_checkPermissionReactChatV2`(pnChannelId           BIGINT(20)
                                                                   ,pnUserId              BIGINT(20)
                                                                   ,pvMessageUid         VARCHAR(100)
                                                                   ) RETURNS DOUBLE
BEGIN
  ...
  END IF;
  -- R1
  SELECT ifnull(max(cm.id), 0) > 0, cm.view_chat_history, cm.join_time
   ...
   WHERE cc.id = pnChannelId
     ...
     ;
  --
  IF nReturn > 0 AND ifnull(nViewHistory, 0) = 0 THEN
    --
