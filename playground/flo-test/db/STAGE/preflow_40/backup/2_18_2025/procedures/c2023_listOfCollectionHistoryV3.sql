...
BEGIN
 ...
 -- DEFAULT IS ASC
  ...
  SELECT ch.id AS id, ch.collection_activity_id, ch.email, ch.action, ch.action_time, ch.content
        ...
        ,ch.kanban_id
       -- ,CONVERT(permistion.object_uid USING cp1251) pobject_uid
       -- ,CONVERT(ca.object_uid USING cp1251) cobject_uid
       -- ,CONVERT(ca.object_type USING cp1251) cobject_type
    ...
    JOIN (
          ...
          SELECT co.id collection_id, co.calendar_uri owner_calendar_uri, co.user_id owner_user_id
                ...
                ,0 member_user_id
                ,usr.username owner_username
                ,concat('KANBAN_', kb.id) object_uid
            FROM collection co
            JOIN user usr ON (co.user_id = usr.id)
            JOIN kanban kb ON (kb.collection_id = co.id)
           WHERE co.id = IF(nCollectionId > 0, nCollectionId, co.id)
             AND co.is_trashed = 0
             AND kb.is_trashed = 0
          ...
          SELECT 0 collection_id, '' owner_calendar_uri, pnUserId owner_user_id
                ...
                ,'NA' object_uid
                            ...
                            --
                            AND ((ch.kanban_id = 0 AND (permistion.object_uid = ca.object_uid OR permistion.object_uid = 'NA'))
                              OR (ch.kanban_id > 0 AND (ca.object_type = 'KANBAN' AND permistion.object_uid LIKE 'KANBAN_%')))
                            --
