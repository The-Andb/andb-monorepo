...
BEGIN
  ...
  SELECT ifnull(max(cn.id), 0), cn.collection_id, cn.channel_id, cn.user_id = pnUserID
    INTO nID, nCollectionID, nChannelID, isNotiOwner
  ...
  END IF;
  -- 2. CHECK permission
  SET nPermission = c2023_checkCollectionNotificationPermistion(nCollectionID, nChannelID, pnUserId);
  --
