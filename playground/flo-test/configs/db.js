...
class DatabaseService extends IDatabaseService {
  ...
  getDBDestination(env, mail = false) {
    const allEnv = [
      {
        envName: exports.ENVIRONMENTS.LOCAL,
      ...
      {
        envName: exports.ENVIRONMENTS.UAT,
      ...
      {
        envName: exports.ENVIRONMENTS.STAGE,
  ...
  getSourceEnv(envName) {
    if (envName === exports.ENVIRONMENTS.DEV) return exports.ENVIRONMENTS.LOCAL;
    if (envName === exports.ENVIRONMENTS.UAT) return exports.ENVIRONMENTS.DEV;
    if (envName === exports.ENVIRONMENTS.STAGE) return exports.ENVIRONMENTS.UAT;
    if (envName === exports.ENVIRONMENTS.PROD) return exports.ENVIRONMENTS.STAGE;
   ...
   */
  getDestEnv(env) {
    if (env === exports.ENVIRONMENTS.LOCAL) return exports.ENVIRONMENTS.DEV;
    if (env === exports.ENVIRONMENTS.DEV) return exports.ENVIRONMENTS.UAT;
    if (env === exports.ENVIRONMENTS.UAT) return exports.ENVIRONMENTS.STAGE;
    if (env === exports.ENVIRONMENTS.STAGE) return exports.ENVIRONMENTS.PROD;
  ...
  getDBName(env, isDbMail = false) {
    if (isDbMail) {
      ...
      if (env === exports.ENVIRONMENTS.UAT) return process.env.UAT_DB_MAIL || '';
      if (env === exports.ENVIRONMENTS.STAGE) return process.env.STAGE_DB_MAIL || '';
    ...
    if (env === exports.ENVIRONMENTS.UAT) return process.env.UAT_DB_NAME || '';
    if (env === exports.ENVIRONMENTS.STAGE) return process.env.STAGE_DB_NAME || '';
  ...
  replaceWithEnv(ddl, destEnv) {
    if (destEnv === exports.ENVIRONMENTS.UAT) {
      return ddl.replace(/@flodev.net/, '@flouat.net');
    } else if (destEnv === exports.ENVIRONMENTS.STAGE) {
      return ddl.replace(/@flouat.net/, '@flostage.com');
    } else if (destEnv === exports.ENVIRONMENTS.PROD) {
