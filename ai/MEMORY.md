# 🧠 TheAndb - AI Memory

> Long-term memory for AI sessions. Update at end of each session.

---

## 🏗️ Architecture Overview

- **Monorepo**: `core`, `ui`, `cli`, `landing`
- **Core principle**: All logic in `@the-andb/core`
- **New Core Engine**: `@the-andb/core-nest` (Framework/TypeScript) - _In Development_
- **UI**: Electron + Vue 3, presentation only
- **Database focus**: MySQL schema comparison & sync
- **Current Phase**: Phase 3 — Expansion (Stable Release & Advanced Features)

---

## 📐 Coding Conventions

- TypeScript for UI and new core code
- Vue 3 Composition API (`<script setup>`)
- Framework for Core (Modules, Services, DI)
- Tailwind CSS v3 for styling
- All text must be i18n localized
- Credentials encrypted via `core/src/utils/crypt.ts`
- Error handling: Result-style `{ success, data?, error? }`
- No `console.log` in core
- **UX Preference**: Avoid Modals unless absolutely necessary. Prefer integrated views, overlays, or sliding panels.

---

## ⚠️ Critical Safety Rules

- DDL = implicit commit, NO rollback guarantee
- Use `GET_LOCK` for MySQL advisory locks
- Pre-flight checks: replication lag, long queries, disk space
- Schema diffing MUST be AST/semantic-based
- DRY-RUN by default for all DDL operations
- **Twin Engine Refactor**: `core-nest` must maintain strict 1:1 parity with `core`. Verify with "Mirror Tests".

---

## ❌ Approaches Tried & Rejected

<!-- Document failed approaches here to avoid repeating them -->

| Date    | Approach             | Why Rejected                                |
| ------- | -------------------- | ------------------------------------------- |
| 2026-01 | File-based locks     | Not reliable in multi-process, use GET_LOCK |
| 2026-01 | Text-based diff      | Not semantic, use AST parsing               |
| 2026-01 | Business logic in UI | Violates layering, keep in core             |

---

## 💡 Key Learnings

<!-- Important discoveries to remember -->

- Dump files are static, cannot migrate/backup
- DBA operations need restriction for dump sources
- Parser must handle custom delimiters (triggers, procedures)
- Deep before Wide strategy — master MySQL first
- Storage: FileStorage (CLI), SQLiteStorage (UI), HybridStorage (Pro)
- **Store Init (Pinia)**: Use `initPromise` guards to prevent race conditions during concurrent `reloadData()` or component mounting. Overwriting `ref` state during slow loads is a major source of bugged "ghost items".
- **Refactoring Strategy**: "Twin Engines" (Side-by-side build) is safer than in-place refactoring for critical core logic.
- **Strategy Pattern**: Essential for supporting Multiple Drivers (`MysqlDriver`, `DumpDriver`) transparently behind `IDatabaseDriver`.
- **Offline Comparison**: `DumpDriver` + `ComparatorService` allows comparing large SQL dumps efficiently without a running database.
- **Dynamic Vite Markdown**: Mermaid graphs inside Vue Markdown (`unplugin-vue-markdown`) require manual DOM extraction and client-side rendering (using `mermaid.render()`) in `onMounted` rather than expecting native server-side transformation.
- **Scrolltelling Robustness**: CSS `background-attachment: fixed` severely breaks Safari/iOS element rendering when nested under elements utilizing `will-change` or `transform`. Implement `sticky top-0` layered card stacks for foolproof cross-browser parallax instead.

---

## Design Patterns in Use

- **Strategy**: Database drivers (MySQL, Dump)
- **Factory**: `DriverModule` (`useFactory`), `ConnectionFactory.create(config)`
- **Singleton**: Connection pools, Logger
- **Observer**: Migration progress, long operations
- **Dependency Injection**: Framework (Core)

---

## 📂 Key Reference Files

| File                                  | Purpose                       |
| ------------------------------------- | ----------------------------- |
| `ai/the-flow.md`                      | AI setup & workflow guidebook |
| `plans/ARCHITECTURE.md`               | Storage strategy              |
| `plans/MAIN_PLAN.md`                  | 2026 roadmap                  |
| `plans/core/CORE_IMPROVEMENT_PLAN.md` | Core engine plan              |
| `plans/QUALITY_CONTROL_PLAN.md`       | Testing strategy              |
| `plans/core/CORE_PARITY_MAP.md`       | Legacy vs Nest parity status  |
| `.cursorrules`                        | Full AI rules & safety checks |

---

## 📅 Session Log

| Date       | Session Summary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-01-26 | AI folder structure initialized with full context from AI.md, .cursorrules, and plans/                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-01-26 | Implemented dump comparison Phase 2: added `isTargetDump` guard with toast to `openBatchMigrateModal()`, added i18n (EN/VI)                                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-01-27 | Fixed `DumpDriver` trigger parsing, added bulk actions to global templates, and verified via E2E/Unit tests.                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2026-01-27 | Resolved Connection Duplication bug via `initPromise` store guards and explicit project targeting in `addConnection`.                                                                                                                                                                                                                                                                                                                                                                                           |
| 2026-01-27 | Analyzed connection duplication & project isolation feedback. Updated PLAN.md with 3-phase fix: Deduplication, Isolation, UI Refinement.                                                                                                                                                                                                                                                                                                                                                                        |
| 2026-01-29 | **Framework Migration**: Initialized `core-nest` with "Twin Engine" strategy. Ported `DDLParser`, implemented `MysqlDriver` & `DumpDriver`.                                                                                                                                                                                                                                                                                                                                                                     |
| 2026-01-29 | **Core Logic**: Implemented `ComparatorService` & `MigratorService` for Tables. Verified successfully on real-world SQL dumps.                                                                                                                                                                                                                                                                                                                                                                                  |
| 2026-01-29 | **Full Parity**: Fixed ESLint, ported CLI commands (`generate`, `helper`), and extended comparator to support Views, Procs, and Triggers.                                                                                                                                                                                                                                                                                                                                                                       |
| 2026-02-04 | **Restricted User Flow**: Implemented `generateUserSetupScript` in `core` (Strategy pattern) and exposed via IPC/Preload. Refactored UI to fetch script from backend for transparency and security-first UX. Fixed SQL syntax for MySQL grants.                                                                                                                                                                                                                                                                 |
| 2026-02-04 | **UI/UX & i18n**: Completed comprehensive i18n overhaul for `Dashboard.vue`, `ProjectSettings.vue`, and `GlobalSchemaView.vue`. Standardized terminology (Project -> Base in Vietnamese). Polished Settings layout with dynamic headers.                                                                                                                                                                                                                                                                        |
| 2026-02-04 | **Architecture Planning**: Initiated "Inherit & Protect" Connection Model. Decided to move all security/infrastructure config (Host, User, Pass, SSH) to Global Level (Templates) and enforce inheritance at Project Level.                                                                                                                                                                                                                                                                                     |
| 2026-02-23 | **CLI Decoupling & Playground**: Decoupled CLI (`nest-commander`) from `@the-andb/core` into `@the-andb/cli` to enforce architectural boundaries. Implemented `andb playground` for local schema diffing. Fixed edge cases in diff engine (reversed Desired/Current schemas, ignored implicit `USING BTREE` in indexes) and added comprehensive integration tests.                                                                                                                                              |
| 2026-03-04 | **The \"Roasting\" Session**: Performed a critical UX and technical debt audit. Identified \"Sidebar Bloat\" as a major DOM performance bottleneck. Implemented **Sidebar Depth Optimization** (limiting tree to category level). Planned the \"Specialized Orchestrators\" refactor for `andb-core`.                                                                                                                                                                                                           |
| 2026-03-04 | **Core Safety & Orchestration Refactor**: Implemented `SafetyReport` engine (SAFE/WARNING/CRITICAL classification) in `MigratorService`. Refactored `OrchestrationService` to pure dispatcher. Relocated `testConnection` to `SecurityOrchestrator`. Enhanced `SchemaOrchestrator` with structured dry-run and decomposed migration logic. 17/17 unit tests pass.                                                                                                                                               |
| 2026-03-04 | **Guard Plan Integration**: Adopted strategic repositioning from "DB Compare Tool" → "Production Safety System" (ADR-010). Integrated `guard.plan.md` into `MAIN_PLAN.md` and aligned Phase 3 with Drift Detection + Deployment Guard goals.                                                                                                                                                                                                                                                                    |
| 2026-03-11 | **Go to Definition & Telemetry**: Implemented Cmd/Ctrl+Click navigation for SQL identifiers across all viewers (`DDLViewer`, `MirrorDiffView`, `SchemaTreeMode`, `DDLDetailModal`). Centralized highlighting logic in `navigation.ts` with Regex caching. Disabled local PostHog telemetry to prevent CSP errors in dev.                                                                                                                                                                                        |
| 2026-03-11 | **Stable Release v4.0.0 (TheAndb)**: Officially transitioned from Beta to Stable. Bumped all package versions (@the-andb/core/cli/mcp to 4.0.0, desktop to 3.1.0, www to 1.1.0). Finalized branding as **TheAndb**. Published stable Core/CLI to NPM. Successfully built and released Desktop v3.1.0 to GitHub. Updated all internal documentation and synced all repositories.                                                                                                                                 |
| 2026-03-12 | **Nuanced Licensing Transition**: Re-licensed Core and CLI under **GPLv3** (Open Source) to ensure engine freedom. Established **Proprietary** status for the Desktop App to maintain premium UI/UX. Updated `andb-www` messaging to reflect "Open Core, Premium UI" and added "No Data Export" privacy commitment.                                                                                                                                                                                             |
| 2026-03-24 | **Open-Source Web Paradigm & Safety**: Vastly simplified the `andb-www` portal into a technical 10-step full-width parallax narrative. Integrated full historical markdown parsing for releases dynamically utilizing Vue hooks. Injected core stability measures into `andb-desktop` including a robust explicit-type-to-delete Project Protection architecture to completely avoid accidental deletions.                                                                                                      |
| 2026-03-30 | **Interactive ERD & Progress Sync**: Overhauled Schema Diagram with a 6-column deterministic Masonry layout (A-Z sorted). Fixed "zombie" progress indicators in `App.vue` via Contextual ID Adoption.                                                                                                                                                                                                                                                                                                           |
| 2026-03-30 | **Unified Professional UI**: Synchronized the "Professional Redesign" (Search/Toolbar/Filters) from Schema View to Compare View. Implemented premium VS-Code style icons and rounded-xl aesthetics globally.                                                                                                                                                                                                                                                                                                    |
| 2026-05-06 | **Agent Orchestration & Skills**: Structured and cooked custom `.claude/skills/` files (`the-andb-core` and `the-andb-desktop`) to guide agentic workflows on core business logic and Electron/Vue components. Registered skills in `CLAUDE.md` and `AGENTS.md`.                                                                                                                                                                                                                                                |
| 2026-05-12 | **Persistence & Isolation Architecture Fix**: Formally introduced `setProjectBaseDir` to `ICoreStorageStrategy` and refactored `DesktopStorageStrategy` to remove brittle `(any)` overrides. Fixed `initialize` signature bug that caused filesystem container derivation default back to system root. Reinforced clean architectural boundaries and enforced isolation across backend worker threads.                                                                                                          |
| 2026-06-08 | **Git Path Isolation & Pattern Healing**: Prioritized `projectBaseDir` for Git storage path resolution in `Integrations.vue`, resolving incorrect project-folder generation (`flo-api-5fcc440fc` vs `/projects/flo_api`). Strengthened `legacyPattern` check in `core-worker.ts` to match 8-36 hex char suffixes. Merged GitNexus rules into `.cursorrules`.                                                                                                                                                    |
| 2026-06-08 | **DDL Definer Lifecycle & Git Status Refactoring**: Exposed `definer` in `getDDLObjects` (`storage.service.ts`). Added "Ignore DEFINER" toggle in `MirrorDiffView.vue` settings dropdown (split/unified) to clean definers from diff display. Restructured frontend Git status layout in `Integrations.vue` to group changed objects by status (staged/modified/untracked) with custom styled tags (TABLE/VIEW/etc.) and healed path configuration fallback in `prepareConfig` (`git-orchestrator.service.ts`). |
| 2026-06-08 | **SQLite Definer Schema Migration & Desktop Rebuild**: Fixed missing `definer` column error in existing `ddl_exports` tables by adding a self-healing `ALTER TABLE` schema migration during SQLite initialization. Recompiled `andb-core` and re-bundled the Electron `core-worker.cjs` using `build:electron` to propagate the changes.                                                                                                                                                                        |
| 2026-06-08 | **Stale CLI Bundle Definer Error Fix**: Re-bundled `andb-cli` via `pnpm --filter @the-andb/cli build` to propagate the newly compiled `@the-andb/core` containing the `definer` migrations and entity definitions, resolving the CLI error `table ddl_exports has no column named definer` during export.                                                                                                                                                                                                       |
| 2026-06-10 | **Pre-Init Definer Healing & Database Sync**: Added pre-initialization schema healing for the missing `definer` column on the `ddl_exports` table in `base-storage.strategy.ts` to execute prior to TypeORM `initialize()`. Manually altered the active database `/Volumes/FlexibleWorkplace/side-pr/TheAndbData/andb-storage.db` to add the `definer` column immediately and restarted the Electron background worker process to apply the schema changes and prevent crashes.                                 |
| 2026-06-10 | **Dogfooding Template Synchronization**: Added the missing `definer` column to the `ddl_exports` table definition in `schema_template.ts`. Recompiled and rebuilt the workspace to synchronize the golden template with TypeORM entity metadata, resolving issues where the Dogfooding Auto-migration Engine failed to match the schema. |
| 2026-06-10 | **Target Environment Fetch Fix**: Updated `AndbBuilder.execute` in `andb-builder.ts` to dynamically resolve the active connection based on the requested `environment` or `env` parameter in options. This ensures that the destination environment DDLs are fetched from the correct destination database instead of incorrectly targeting the source database during comparison. |
| 2026-06-11 | **DDL Introspection Comma Repair, Migration Reporting & Regex Parser Fix**: Fixed index parsing and DDL generation in `mysql.introspection.ts` by making it case-insensitive, backtick-optional, and robust against trailing whitespace/carriage returns (`\r\n`). Corrected `performLiveMigration` in `schema-orchestrator.service.ts` to return `success: failed.length === 0`. Also fixed DDL body extraction in `ParserService.parseTableDetailedRegex` of `parser.service.ts` by replacing `lastIndexOf(')')` with a balanced parenthesis scanner, preventing incorrect column/index extraction when table options contain parentheses (e.g. `PARTITION BY`). |
| 2026-06-11 | **Dialect-Specific Index Parsing Fix**: Fixed a bug where standalone SQLite-style `CREATE INDEX` definitions were incorrectly generated in the fallback regex parser (`parseTableDetailedRegex` in `parser.service.ts`) regardless of the database dialect. Modified the method to receive the `dialect` parameter and only reconstruct standalone SQLite index syntax when the dialect is SQLite. For MySQL, it preserves the inline MySQL-style index definitions, ensuring the generated `ALTER TABLE ... ADD KEY` migration DDL statements are syntactically valid. |


